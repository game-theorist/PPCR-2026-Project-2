---
title: "PPCR 2026 - Matheus Petroni - Group 12 - Assignment 16"
format: html
editor: source
execute:
  warning: false
...
**1. Set up your work for this week in two parts.** 

**a) Restate your model from last week in two or three lines: your primary outcome and its type, your main exposure, and the model you are using (linear or logistic). **

Our primary outcome is depression as defined by PHQ-9 score of 10 or higher. It is a categorical, binary outcome. 
The main exposure is sitting time (measured in hours of sitting time per day). We expect that subjects with higher sitting time could have a higher prevalence of depression.
A logistic regression will be used.


**b) List the candidate confounders you will test — the variables you suspect may be distorting your exposure–outcome relationship. Select at least eight candidate confounders, and give half a sentence on why you suspect each one**

Sitting time (measured in hours of sitting time per day) - Main exposure. We expect that subjects with higher sitting time could have a higher prevalence of depression.

Gender(female = 0, male = 1) - Confounder.  e.g. if one gender has different habits compared to the other, such as staying at home longer or longer distances commuting to work, this could affect sitting time. As for depression, globally women have higher rates of depression compared to men.

Race(Non-Hispanic White = 0, Mexican American = 1, Other Hispanic  = 2, Non-Hispanic Black = 3, Non-Hispanic Asian = 4, Other Race - Including Multi-Racial = 5) -  Confounder. Similar to gender. 

Unemployment(employed = 0, unemployed = 1) - Confounder. Similar to race and gender.

Smoking status (non-smoker = 0, smoker = 1) - Confounder. Smokers may care less about following a healthy lifestyle and thus have higher sitting times, as well higher rates of depression due to less awareness about measures for maintaining a better mental health.

Alcohol use (non-drinker = 0, drinker = 1, heavy drinker = 2) Confounder. Similar to smoking status.

Comorbidity burden(no comorbidities = 0, one comorbidity = 1, two or more comorbidities = 2) - Confounder. Subjects with a higher number of comorbidities could be more restricted and less functional, leading to higher sitting times. Also, comorbidities could lead to lower life satisfaction and thus depression.
...
**4.1 Interpret the effect of your main exposure in the adjusted model. For linear regression: what does the β mean, in the units of your outcome? For logistic regression: what does the odds ratio mean? Is it statistically significant (look at the p-value and the 95% CI)?**

For the adjusted model, the coefficient (estimate) of 1.05 for sitting_time means that, for every hour a person spends sitting per day, their odds of having depression increase by 5%(considering the confidence interval, we're 95% confident that it increases between 3-8%).

**4.2 Compare the crude versus adjusted estimate for your exposure. Did it change substantially? A change of roughly 10% or more is often taken as a sign of confounding. What does that tell you about your confounders?**

It did not change substantially. This tells that theres not much confounding happening, at least for the chosen variables.

**4.3 How well does the model fit? For linear regression, report R² / adjusted R² and explain what it means. For logistic regression, comment on whether the overall model is significant.**

The overall model is significant. The intercept and variable coefficients such as sitting time, gender, smoking and unemployment all have very low p values, attesting to the models significance.

**4.4 In two or three sentences: does this preliminary model answer your research question? What is one thing you will discuss with your group to improve it?**

This model does not answer our research question because our question involves specifically using physical activity data (as in following the WHO guideline recommendations for physical activity) as an effect modifier for sitting time. As we were instructed to not use effect modifiers for this assignment, the question still remains.
